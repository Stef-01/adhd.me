# ADHD.ME BUILD-STATE — claim ledger + protocol
...
> **PLAN REORIENTED 2026-08-14 — FORWARD ONLY.** `docs/FIVE-YEAR-PLAN.md` had never been reoriented,
> so the loop was executing unit definitions written for the PMOS product: W56's remaining values
> were diabetes and CKD intervals, W161–W163 cardiometabolic and CKD pathway content, W186
> dermatology, W250/W251 respiratory. The rule applied was the one this banner already states —
> **rewrite the future, never the past.** Units already `done` keep their original wording; unbuilt
> units whose domain was unambiguously wrong were reoriented to ADHD assessment and marked
> `[REORIENTED 2026-08-14]`, keeping their ID, `[P]` marker, verify gate and founder gate. Three
> units name a clinical vertical that is a product decision rather than a translation (W186, W250,
...
This repo — `stef-01/stef-01-adhd-me-`, branch `main` — is the permanent home (W-MIGRATE
completed 2026-08-08; the tree previously lived in Stefan-Brain `adhd-me/`, now a pointer).
...
> tree.
> **THE FAILURE THIS UNIT HAS TO DESIGN AGAINST IS SCOPE AND TENANCY COLLAPSING INTO EACH OTHER.**
> W253 answers WHICH PRACTICE — resolved from memberships, minted as a brand no caller can
> construct. A scope answers a different question: WHAT KIND OF READ, within a practice the caller
> already holds. They are independent axes, and the whole class of authorisation bug worth fearing
> here is one silently doing the other's job: a scope that also widens tenancy, or a tenancy check
> that gets treated as sufficient because the caller "has access". Y4-1 was one axis failing while
> the other looked fine. So authorising a read has to require BOTH, structurally, and neither may
> be derivable from the other.
> **AND A SCOPE MODEL FAILS QUIETLY IN A WAY A TENANCY MODEL DOES NOT.** An unscoped tenancy read
> returns somebody else's rows and a test that seeds two practices sees it. A scope that grants
> more than it should returns exactly the right practice's data through the wrong door, and every
> assertion about tenancy passes. What catches it is the census check in BOTH directions — every
> endpoint requires a declared scope, and every declared scope is required by some endpoint — plus
> the property that makes it worth having: **a caller granted nothing can read nothing**, which is
> the assertion a permissive default fails and a spot check on a granted caller never reaches.
> **"NO PRODUCTION CREDENTIAL ENTERS THE TREE" IS W242's SHAPE AND W242's WARNING.** That unit drew
> the line already: "there are no credentials in the tree" is a fact about today's contents and
> stops being true in a commit that looks like configuration; "nothing can be constructed while the
> gate is shut" is a property of the code. The same applies here with one difference worth stating
> — G1 covers PMS and booking credentials, and **this is a credential for OUR OWN API**, which no
> named gate covers. The honest posture is not to claim a gate protects it: nothing is shipped,
> nothing can be minted, and the grant registry is pinned empty by its own test.
> Gate: scopes declared as data with what each grants and why it is separate from the others; the
> census checked BOTH directions against W253's endpoints; authorisation requiring a resolved
> practice AND a granted scope, with neither derivable from the other — asserted by a test that
...
>
> **DONE 2026-08-21T11:51Z at `8a52c57`.** Third vertical is `vert-undecided-3` in
> `src/verticals/third.ts` — two members, the sparsest of the three, so the report's coverage
> statement is exercised on a bundle whose kinds are a strict subset. It cost one declaration and
> no machinery, which was the point of building it after W248 rather than before.
> **THE RECONCILIATION IS THE UNIT, AND W158's OWN SENTENCE IS WHAT NEEDED CORRECTING.** It read:
> the chain is named per kind, "true of every member of that kind and cannot drift". First half
> right and kept. Second half wrong, and wrong in a way worth keeping on the record: **static is
> not the same as unable to drift.** `REMAINING_CHAIN` could not drift from itself. It drifted from
> the declaration layer that grew up beside it — and had already: the report told a founder an
> education item needs *"a signed-off source for it to cite"* while every declared education member
> says no founder gate applies to it at all. The declared act now wins where there is one and is
> CARRIED rather than restated, and the grouping key moves from the kind to the act, which also
> fixes the collapse W248's own second vertical exposed.
> **AND THE "SPECIALIST" HALF IS A CORRECTION TO MY OWN CLAIM, WHICH IS THE PART WORTH READING.**
> I wrote that the tree "had already ruled on that", implying an oversight. It had not been an
> oversight: **W200 holds an ACCEPTED finding for exactly this string**, reasoned — the rule bundles
> the word because a clinician CLAIMING the title is a prohibited claim, and someone reviewing our
> own content is the opposite direction of the same word — with a review date in 2027. So the tree
> ruled TWICE, in opposite directions: W191 removed it from the declarations, W200 accepted it in
> the report. The inconsistency was real and my account of how it got there was not.
> **What justifies the change is the ACCURACY argument, which stands whatever one thinks of the
> compliance one**: W119's chain is a reviewer and then a signatory who is not the reviewer, and it
> does not require the reviewer to hold a protected title. The report told a founder that it did.
> Retiring the acceptance is a side effect rather than the reason — and the thing that surfaced it
> was **W200's own rule that every acceptance must still be live**, which fired the moment the
> string it accepted stopped existing. A register catching a stale exemption is exactly what W247
> was arguing for one unit earlier.
> **THEN THE FIRST REWRITE TRADED ONE BANNED PHRASE FOR ANOTHER.** Replacing "whether a specialist
> has looked" with "whether a reviewer has looked" walked straight into W158's own ban on stage
> vocabulary — `reviewer has` is on the list. Two rules over one sentence, and satisfying one is
> not satisfying the other. The footer now says how far along W119's chain a member has got, and
> the ban list carries BOTH words so the next edit cannot pick either.
> **Three seeded failures**: the declared acts not carried through, the protected title restored to
> one chain, and a regrouping that loses members (caught by the totals check, the golden report and
> the per-act count independently).
> Verification: `pnpm verify` green (246 files, 3976 tests, audit gate 2 accepted / 0 unaccepted);
> `e2e/verticals.spec.ts` 9/9 — one assertion had pinned the old wording and was updated to carry
> W119's roles instead, so it fails if the overstatement returns. Vault log skipped — Stefan-Brain
> unreachable.
...
>
> **DONE 2026-08-21T11:33Z at `c929551`.** The vertical is `vert-undecided-2`, "Second care area
> (undecided)", in `src/verticals/undecided.ts`. It names no condition.
> **THE CONTRADICTION IS RAISED, NOT RESOLVED, AND THE PLAN BANNER WAS WRONG ABOUT ITS OWN SCOPE.**
> The banner said THREE unbuilt units name a vertical that is a product decision (W186, W250,
> W251). It is five: W248 and W249 are titled with the pre-reorientation domain and were never
> reached. Corrected there and in W249's row — **and neither correction picks a care area**, which
> is the line the loop does not cross. Recorded as the SEVENTH outstanding founder item.
> **"REUSED, NOT RE-IMPLEMENTED" WAS THE CLAUSE WITH A TRAP IN IT.** Copying `dermatology.ts` and
> changing the ids *does* reuse W157's model — and duplicates the four functions around it: two
> evidence assemblers reading the same registries, two outstanding-work wrappers, two gate lists.
> A second instance is when an abstraction stops being speculative, so `declareVertical` is now the
> one door and both verticals go through it; dermatology's 96 tests pass unchanged, which is what
> makes it a refactor rather than a rewrite. Evidence moved with it, on an argument worth keeping:
> **what is signed off is a property of the TREE, not of a bundle**, and giving each vertical a
> place to hold its own answer would invent a distinction that does not exist.
> **THE SHARING IS ASSERTED STRUCTURALLY, BECAUSE A COMMENT CLAIMING IT PROVES NOTHING.** The
> second vertical rebuilds from its declaration alone and matches; both expose the same shape; the
> module's source is scanned for a second assembly path. And its membership differs from
> dermatology's on purpose — six members, a different mix, and the case dermatology lacks (two
> members of one kind waiting on DIFFERENT acts) — because a second bundle of the same shape would
> prove only that the machinery works twice on one shape.
> **A SEEDED FAILURE CAUGHT MY OWN TEST BEING WEAKER THAN ITS NAME.** It was called *"would become
> usable if the evidence existed"* and never supplied any: it compared member counts between the
> two verticals, which catches a hardcoded refusal and sails straight past an assembly that IGNORES
> the evidence handed to it. I seeded exactly that — and **W191's dermatology suite caught it while
> this unit's did not.** The test now supplies one member's evidence and requires the refusal to
> move. Third time this session a guard has turned out weaker than the sentence standing over it,
> and the first time an older unit's suite was the thing that noticed.
> **ZERO CLINICAL CONTENT IS READ, NOT RE-ASSERTED.** W158 already computes
> `noSignedOffClinicalContent`; a fresh assertion here would be a second claim about the same fact,
> and the second claim is the one that drifts. Its non-vacuity probe THREW before it reported, on a
> fake too fake to survive the report's own field reads — worth recording because a probe that
> crashes proves nothing either, and this one came close to being abandoned as unwritable.
> **Five seeded failures**: a second assembly path in the module, a member ref naming the domain,
> an assembly ignoring its evidence, a refusal naming only the first missing member, and that same
> seed re-run against the strengthened test.
> Verification: `pnpm verify` green (245 files, 3963 tests, audit gate 2 accepted / 0 unaccepted);
> `src/verticals/declare.ts` and `undecided.ts` both registered in W200. Vault log skipped —
> Stefan-Brain unreachable.
...
> about the Double Bay rooms invented — no practice name, no separate booking. He is
> telehealth-first, so the distance machinery's posture is unchanged BY DESIGN: the
> sentence stays "by telehealth, wherever you are" and the near-sort keeps his fit
> position (O85's nearest-location logic never applies to somebody nobody travels to) —
> what changes is the LABEL: row and profile show "Beecroft & Double Bay". Gate: §O86
> unit pins (label pair; telehealth sentence unchanged with a second location present;
> near-sort exemption held), e2e row assertion with qa/location-o86 capture + DESIGN-QA
> entry, full `pnpm verify` green.
...
> every claim probe-backed), §O78 pins both directions, 411-entry corpus green, floors
> titration 18→19 + sense_making 13→14 + bulk-billing 15→16 on the fix's own pins,
> tie-quality repinned 279/175/22/82, full `pnpm verify` green (211 files, 3231 tests).**
> FIXED (1): suppression was accidentally SENTENCE-GLOBAL — findCue returned only the first
> occurrence, so a cue negated/hedged/on-behalf-governed in clause one was dead for the
> whole text ("I don't want titration. but titration support is exactly what I came for"
> read as nothing). findCue now takes a start index and readNeeds retries past a refused
> span; occurrence-local refusals retry, sentence-global ones (O45 collapse pair, seen
> facet) end the search. The standing 405 entries were BEHAVIOUR-IDENTICAL under the fix —
> zero pin movement — the strongest evidence it widens recall only where the old behaviour
> was accidental. FIXED (2): unservedAsks was the last reader of the global roster; now
> injectable per the O8 rule. NAMED for their own units, carried as corpus gap data with
> design sketches in the audit doc: desire-negation over-scope ("…woman GP, bulk billing
> matters more" — consume-once scope, NOT a shorter lead, which would break "don't want
> anyone touching the dose"); reported refusal ("they said no to titration and I want it
> anyway"); the sit-in-the-room presence phrasing O25's re-authoring left unheard.
> VERIFIED SOUND with probes: W213 unity to the thousandth on adversarial mixed input;
> roster-permutation variance confined to within-band ties (the documented non-ranking).
> REFUSED with written reasons: raw-map precompute and needsFor caching and first-token
> indexing (0.049 ms/call measured — complexity for a twentieth of a millisecond), Porter
> (O50's law), maximum matching (already priced by the oracle).
...
| W19 | done | builder-B | 2026-08-08T22:40Z | 680fe91 | ops switches at src/ops honored in sim (kill/pause → 0 sends); /console/ops queue + toggles gated by pause_sending (W18); global kill-switch uses practice pause_sending grant in synthetic phase (ADHD.ME-superadmin role deferred) |
...
| W161 | blocked | — | — | — | **FOUNDER GATE G5 — not buildable by any builder.** **[REORIENTED 2026-08-14: was "Cardiometabolic pathway content".]** Adult ADHD assessment pathway content → verify: two-person sign-off recorded per W119. **Why blocked:** unchanged by the reorientation — this is clinical content: inclusion, exclusion and escalation criteria a GP would act on. W119's two-person sign-off exists and has never been used, because a specialist reviewer and a founder signature are people, not code. Changing which condition the unit names does not make its content authorable by a loop. |
| W162 | blocked | — | — | — | **FOUNDER GATE G5 — not buildable by any builder.** **[REORIENTED 2026-08-14: was "Early-CKD pathway content".]** Child and adolescent ADHD assessment pathway content → verify: as W161. **Why blocked:** same reason as W161. Listed separately for the reason the original pair were: the two fail independently, one may be signed off without the other, and a plan that bundles them would hide that. The age split is where Australian practice diverges, so it is the right seam. |
| W163 | blocked | — | — | — | **FOUNDER GATE G5 — not buildable by any builder.** **[REORIENTED 2026-08-14: was "Cardiometabolic/CKD interval values".]** [P] ADHD review-interval values — W56's question at vertical scale → verify: every interval carries a citation and a review date. **Why blocked:** Interval VALUES. W56 shipped the container in Q5 and the values have been blocked ever since; this is the same question at vertical scale. A loop that picked a review interval would be authoring guideline content. |
