...
| Community landing (`app/story/page.tsx`) | founder commits 3317340, cfa2f1d; moved to `/story` by O230 | 2, 3 | Plain-language PMOS awareness copy and the interest form share this route. The form collects name, email and audience only; the privacy link and synthetic-demo separation remain visible. It does not diagnose, rank clinicians or make a treatment claim. The community interest register row below governs storage and retention limits |
...
| Your details (`app/profile/page.tsx`) | O233 | 2 | The Profile tab. Renders only what this device's `sessionStorage` already holds — the described words and the given suburb — plus the control that clears them. Stores nothing of its own, sends nothing, and personalises no clinical content; the privacy page governs the storage itself |
| Clinician finder (`app/page.tsx`) | founder commit 3317340; moved to `/` by O230, which made the app its own front door | 2, 3, **G6, G7** | The care-finder, now served at the root rather than at `/finder` (a 308 since O230). The open G6/G7 questions and the seven P1 defects in `qa/audit-matching-trust/audit.md` apply unchanged. The roster's example profiles are labelled as such on every card and profile, and the whole route is `noindex, nofollow` under the U7 register — the controls that distinguish a testing deployment from a public directory, and both are stronger than a title that said "demo" |
...
| Operations endpoints (`app/api/health`, `app/api/csp-report`, `app/api/vitals`) | U4 (`src/ops/reporter.ts`) | 2 | Three unauthenticated routes that hold nothing personal by construction. `/api/health` discloses the build's commit and boot instant (the commit has been on the voice debug banner since O73) and nothing else. `/api/csp-report` and `/api/vitals` are intakes, not disclosures: each accepts one bounded shape (a policy-violation document; one of three named Web Vitals), refuses everything else with a 4xx, and keeps a path only after the query string is cut — a finder request travels as a query string, and `src/ops/reporter.test.ts` plants one in every channel and proves it absent from every report payload. What reaches the sink is a route, a commit, an error's own text and the router's context, never a header, a cookie or a body |
| Mock introspection routes (`app/api/mock/*`) | W44 (`src/lib/mock-guard.ts`) | 2 | Eight routes that reset and read shared state, and disclose signed booking tokens. `assertMockRoutesEnabled()` 404s them outside a non-production build unless `ADHDME_ENABLE_MOCK_ROUTES=1` is set explicitly; a real deployment sets neither. Synthetic-phase only, removed when real persistence lands. U3 adds `/api/mock/fault/[kind]` under the same guard: a page that throws on request so the e2e can reach the error boundary; it discloses nothing. U4 adds `/api/mock/reports`, which reads the reporter's last fifty reports so the e2e can prove a fixture error reached the sink without the server's stdout; the reports hold paths and error messages, so it sits behind the guard |
...
/api/csp-report — Operations endpoints
/api/health — Operations endpoints
...
/api/mock/fault/[kind] — Mock introspection routes
/api/mock/reports — Mock introspection routes
/api/vitals — Operations endpoints
