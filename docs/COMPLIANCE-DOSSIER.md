...
| Mock introspection routes (`app/api/mock/*`) | W44 (`src/lib/mock-guard.ts`) | 2 | Eight routes that reset and read shared state, and disclose signed booking tokens. `assertMockRoutesEnabled()` 404s them outside a non-production build unless `ADHDME_ENABLE_MOCK_ROUTES=1` is set explicitly; a real deployment sets neither. Synthetic-phase only, removed when real persistence lands. U3 adds `/api/mock/fault/[kind]` under the same guard: a page that throws on request so the e2e can reach the error boundary; it discloses nothing |
