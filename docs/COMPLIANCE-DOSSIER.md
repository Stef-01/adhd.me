...
| The approach page (`app/approach`) | founder commits 95e59bc, 252cd97 | 2, 3 | The landing page's argument at length: the search that returns no GP, practice pages read one doctor at a time, the questions a profile cannot answer, the wait and the bill, the NSW and Queensland rule change, and the three steps. Same audience as `/` and governed by the same W23 linter, which the copy passed before it moved here — splitting it onto its own route relaxed nothing. Carries ONE interactive control, a suburb lookup that reads the `src/geo/suburbs.ts` gazetteer and reports straight-line distance to the listed GPs; it stores nothing, asks for nothing else, and answers an unresolved input as unresolved rather than fuzzy-matching it. The figures it quotes are the `landing-copy.ts` figures and carry the same pending-source qualification in frame |
| Matching console (`app/console/matching`) | W221 | 2, 3 | Staff-facing. Renders the roster's DECLARED tags, re-runs `matchAudit` over a synthetic query, and shows a synthetic interview transcript read into proposals. Holds no state, writes nothing, and reads no patient data — the query is a literal in the page. Every number is read back out of the functions the finder uses, pinned by `src/onboarding/background.test.ts`, so the panel cannot show a score the product does not act on. The assembled bio on this page is generated from accepted facets and is not editable text: `src/directory/profile.ts` refuses a free-text biography, and a console text box labelled "professional bio" would reintroduce that field through the back door |
| Allocation breakdown (`app/console/allocation`) | W236 | 2, 3 | Staff-facing, and BOTH SIDES ARE INVENTED — the page says so on itself in its opening lines. The patient is synthetic by founder gate; the prescribers are synthetic by a second argument, that the module scores an ASSIGNMENT of a patient to a prescriber and the finder deliberately does not do that (it lists, and a person chooses), so rendering an assignment against a real doctor's name would show the product doing something it has decided against. Holds no state, writes nothing, reads no patient data — the patient's words are a literal in the page. It AUTHORS NO VOCABULARY: every weight, sub-score, contribution and explanatory sentence in the tables is read out of `matchPatientsToPrescribers`, where those sentences are declared and linted, so the surface cannot drift into disagreeing with the scorer it displays. The derived vocabulary shown under the patient's words comes from `readNeeds` — the finder's own reader (O132) — so the two never diverge. Stated urgency is rendered as the patient's own statement with the reason beside it: a priority inferred from what somebody wrote would be a clinical judgement, and G7's boundary is exactly there |
| Onboarding interview (`app/console/interview`) | W229 (O30, `src/onboarding/capture.ts`) | 2, 3 | Staff-facing and session-gated. The working instrument behind the matching console's worked example: an interviewer types the conversation, `readTranscript` and the patient reader propose facets live, and each proposal is confirmed with the structured interview's own question before an often/sometimes/not-me answer is recorded. The transcript is internal and never published (`src/directory/profile.ts` refuses bios); NO AUTO-ACCEPT exists — the only path from proposal to declaration is one recorded answer naming the interviewer, enforced at the W226 writer (an accepted facet naming nobody is refused). Saving writes the same draft record the review editor writes; nothing on this path can publish while `SHIPPED_DIRECTORY_PROFILES` stays empty behind G6. Clinician speech is neutralised against spreadsheet formulas at the writer per W153 |
...
| Clinician application (`app/clinicians/join`) | 2026-08-13 design + onboarding pass | 2, 3 | Clinician-facing. Collects only fields a `DirectoryProfile` (W183) can hold, and `REFUSED_APPLICATION_FIELDS` states in the tree which fields are deliberately absent and why: no biography, no years of experience, no certificate upload, no street address, no fee. Asks nothing about any patient. The NSW training is recorded as a DECLARATION, not a credential, because no public register exists to check it against, and every surface that renders it says "says they have completed". Submitting creates a row with the single status `received`; there is no code path to `approved`, because publication is gated on an Ahpra advertising review, which is a human act (G6). Names and free-text fields are neutralised against spreadsheet formulas at the writer per W153 |
...
| The network (`app/network`, `app/network/[clinician]`, `app/mission`) | O192, O197 (founder-directed; `src/network/gallery.ts`, `src/network/mission.ts`) | 2, 3 | The browse-first half of the two interfaces the founder asked for: `/finder` ranks the roster against what a reader describes, `/network` shows the same people and lets them be read. **Same roster, no new fields.** The gallery imports `src/demo/clinicians.ts` and nothing else about a doctor — every sentence beside a named GP is the declaration already served publicly at `/finder`, rearranged rather than authored. It deliberately does NOT import `SHIPPED_DIRECTORY_PROFILES` from `src/directory/`, and `src/network/gallery.test.ts` enforces that on the import lines: minting a `DirectoryProfile` is directory publication and stays behind **G6**, which this surface does not touch. The page's own framing sentences — the only words here that are ours — are held as constants and run through the W23 landing linter at unit speed as well as by the rendered sweep. Each clinician's voice is labelled ("In their words"), the declared interest renders ABOVE the booking link in document order (W193, pinned in `e2e/network.spec.ts`), and the deck states the network's size in words derived from the roster so a two-GP network cannot read as a directory. **Founder decision 2026-08-26** (`network-gallery-ahpra-hold` in `src/design/founder-gates.ts`): the advertising-review hold that had this route shipping `noindex` was lifted by the founder — “we have experts that will do this”. The review is theirs to commission; the copy law above did not move. **O197 added `/mission`, the network's own landing page**, and it is the one place in the tree where a sentence ships that the linters match. The mission statement is the FOUNDER'S, verbatim by their instruction, and the word `best` in it trips W23's `no-superlatives` and W6's `no-benefit-claims`. It is held in its own field (`MISSION_COPY.statement`) and separated from the page's other copy by `FOUNDER_AUTHORED`, so `src/network/mission.test.ts` lints every OTHER sentence on the page with no acceptance behind it and asserts the exclusion is exactly one named sentence rather than a filter that can widen. Both findings are entered in `ACCEPTED_FINDINGS` against `/mission` keyed on (path, rule, match) with a 2026-11-26 review date — the same date as the `/network` entry, so the network and mission copy reach the reviewer together — which makes the rendered sweep REPORT them rather than block the page or reword it. That is a routing decision, not a compliance judgement: **founder decision 2026-08-27** (`mission-copy-authored-by-founder` in `src/design/founder-gates.ts`) puts the advertising review with the experts the founder has engaged, and this loop out of it. The page's own paragraphs state the network's size and the single state it covers, derived from the roster, so ambition is never rendered as coverage |
...
| Clinician applications register (`app/console/applications`) | O57 (`src/onboarding/applications-view.ts`, W233), W105 (`src/tenancy/staff.ts`), W213 (`src/onboarding/store.ts`) | 1, 2 | Staff-facing read of the W213 application rows — a GP's own name, email, registration number and declarations, so the same posture as the interest register: **ADHD.ME-staff-only, gate ABOVE the read** (the list is never fetched for a non-staff visitor, keeping it out of the RSC payload), and the grant list ships empty. Every rendered sentence is composed by W233 and unit-pinned: the Ahpra number renders as an UNCHECKED SHAPE, the NSW training and books answers render as the applicant's answers ("says they"), and the join hero's declared mix renders — only when the row carries it — as stated preference, never a referral promise and never a directory field. Free-text applicant fields render verbatim under the W153 `public_form` attribution. **No approve/publish control exists on the page**: status is `received`, the only value the type allows, because listing is gated on an Ahpra advertising review — a human act (G6 stays closed). Same pre-real-use caveat as the interest register: contact details of people outside any practice tenancy, so W33 retention/erasure and APP 5 notice apply before any real use — synthetic-only today (G2) |
| Launch pages (`app/faq`, `app/examples`, `app/thanks`) | O15 (launch checklist) | 2, 3 | Three prose pages added by the launch pass, all classified `patient` in the W192 registry and swept by the rendered-copy and axe suites. `/faq` answers administrative questions only and generates its `FAQPage` JSON-LD from the same list it renders; `/examples` is the compliant form of a case-studies page — worked examples computed live by the finder's own pipeline over demo scenarios, no patient stories, because a patient outcome used as marketing is a testimonial and both tree law 6 and s133 of the National Law prohibit those; `/thanks` is the post-form page and makes one operational promise (a person replies within two business days), the same sentence the form section carries |
...
| Mock introspection routes (`app/api/mock/*`) | W44 (`src/lib/mock-guard.ts`) | 2 | Eight routes that reset and read shared state, and disclose signed booking tokens. `assertMockRoutesEnabled()` 404s them outside a non-production build unless `ADHDME_ENABLE_MOCK_ROUTES=1` is set explicitly; a real deployment sets neither. Synthetic-phase only, removed when real persistence lands |
| Counsel review notice (`app/privacy/counsel-review`) | O41 | 2, 3 | The draft banners explained on-site: what counsel has been asked to check, in patient-safe words, with the raw briefing pack linked as a public repository document and labelled working-document-not-advice. Banners come down on counsel's word only; the page records the date when they do |
| Team (`app/about`) | O90, gated O155 | 2, 3 | **Gated shut since O155** (founder-directed: hidden while the team is still forming). `TEAM_PAGE_PUBLIC` false → route 404s, `robots: noindex`, no sitemap entry, no footer door. When it returns: supplied portraits and self-declared roles and affiliations only; the h1 is the one word the founder specified; no clinical content |
| Terms of use (`app/terms`) | O39 | 2, 3 | Draft-marked, counsel-gated with the privacy policy (docs/PRIVACY-COUNSEL-BRIEF.md q.10). Plain-language: not a medical service, no advice, 000/Lifeline for urgency, order earned and never paid for, listing is not endorsement, Healthengine terms govern the booking, ACL guarantees expressly preserved, NSW law. Renders the canonical responsibility statement from its constant (W138) so it cannot drift from the privacy page |
| Booking handoff redirect (`app/go/[clinician]`) | O28 (`app/go/[clinician]/route.ts`) | 2 | A 302 from this domain to the clinician's own Healthengine profile, so outbound booking intent is countable per clinician from the platform's request logs. Stores nothing — no cookie, no identifier, no body; the UTM tail names adhd-me as the source to the destination's analytics only. Renders no copy, so no linter scope. robots.ts disallows `/go/` to keep crawlers out of the count. Unknown ids 302 to /finder. Full analysis in `docs/BOOKING-ATTRIBUTION.md` |
...
/clinicians/join — Clinician application
/go/[clinician] — Booking handoff redirect
...
/console/applications — Clinician applications register
...
/console/interop — Weekly/pilot reports
/console/reporting — Reporting summary
/console/privacy — Privacy page
/console/registers — Register console
/console/matching — Matching console
/console/allocation — Allocation breakdown
/console/interview — Onboarding interview
...
/approach — The approach page
/demo — Demo presenter page
/finder — Clinician finder demo
/network — The network
/network/[clinician] — The network
/mission — The network
/examples — Launch pages
/faq — Launch pages
/about — Team
/terms — Terms of use
/thanks — Launch pages
...
/privacy/counsel-review — Counsel review notice
