...
| 14 | Maps + directions | **Deliberately delegated to booking** — doctor profiles do not duplicate maps or directions already shown by the live booking destination. |
...
| 17 | Local schema | **Built, compliant subset** — `Organization` + `WebSite` JSON-LD with current `areaServed` (Sydney) in the layout; `FAQPage` and `BreadcrumbList` per page. Deliberately absent: `aggregateRating`/`Review` (item 15) and `Physician` markup (public directory copy sits behind founder gate G6). |
...
| 19 | Google Analytics | **Built, dark by default, and behind the agreement (U13)** — GA4 loads only when `NEXT_PUBLIC_GA_ID` is set at deploy time (no measurement ID lives in this repository, same posture as gate 4's "no production credentials"; the e2e suite sets a placeholder in Google's format, `G-E2E0000000`, which is not one) AND only after the visitor has agreed to the privacy policy; withdrawing on `/privacy` stops it in the same document and brings the bar back. Advertising signals off. Setting the variable turns on the loader, the privacy section and the policy's GA hosts. |
