...
**The fourteen unenforced taste rules.** Pinned in `UNENFORCED_COUNT`; a green verify says
...
| `type.palette-tokens` | **covered-but-unlinked** — theme-parity's hex ratchet enforces the substance; the register entry simply does not cite it |
...
**The five enforced rules with no mutation probe** (their detectors run, but nothing plants a
...
- **One theme** is covered at dossier time (AR17 light done; AR18 dark claimed by another
  session — its parity is that unit's to prove).
...
| gap | cost | grounds |
|---|---|---|
| Link `type.palette-tokens` to theme-parity | **trivial** (one `enforcedBy` line, one tag, two pin bumps, coverage-doc regen) | AR30 paid exactly this cascade for `honesty.no-testimonials` |
...
| Second-theme parity | not this lane's to price | AR18, claimed by loop-0825e |
