...
test("the storybook's copy is legible with JavaScript disabled", async ({ browser }) => {
  ...
  await legible(page.getByRole("heading", { name: /The same conversation keeps happening/i }), "chapter 1");
  await legible(page.getByRole("heading", { name: /ADHD\.ME helps people reach an assessment/i }), "chapter 5");
