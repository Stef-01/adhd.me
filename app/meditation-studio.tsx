...
export function MeditationStudio() {
  ...
  return <main id="main-content" className={`meditation-studio${session ? " is-playing" : ""}`}>
    <div className="meditation-topline"><Link href="/approach"><ArrowLeft size={18} /> All learning</Link>{session ? <button onClick={close} aria-label="Leave this session"><X size={22} /></button> : <SunHorizon size={26} />}</div>
    <AnimatePresence mode="wait" initial={false}>
      {!session ? <motion.div key="lobby" className="meditation-lobby" initial={reduced ? false : { opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
        <div className="meditation-invitation"><MeditationTitle focus={hasStarted}>A little less doing.<br />A little more being.</MeditationTitle><p>Sit comfortably. Follow the prompts.</p>
        ...
        <span className="meditation-session-label">{complete ? "A moment, taken" : session.mode === "shared" ? "Shared session" : "Your own moment"}</span>
