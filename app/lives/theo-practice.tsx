...
type Phase = "playing" | "paused" | "success" | "failure";
...
export function TheoPractice() {
  ...
  const [phase, setPhase] = useState<Phase>("playing"), [elapsed, setElapsed] = useState(0), [line, setLine] = useState("");
  const phaseRef = useRef(phase), elapsedRef = useRef(0), heading = useRef<HTMLHeadingElement>(null);
  const reduced = mounted && Boolean(preference), still = reduced;
  ...
  }, [phase, still, plan.duration, mounted]);
  ...
  return <section className="theo-practice lives-run" data-phase={phase} aria-labelledby="theo-title">
    ...
    <header className="theo-heading"><h1 id="theo-title" ref={heading} tabIndex={-1}>{phase === "success" ? "And you’re off!" : phase === "failure" ? "Still in the hallway." : phase === "paused" ? "The world can wait." : "Essentials. Then exit."}</h1></header>
      ...
      {result && <div className="theo-result"><p role="status">{line}</p><div><button className="theo-primary" onClick={start}><ArrowCounterClockwise size={20} /> Play again</button></div><Link href="/lives/learn?module=launch_pad_v1">Give your essentials a home. Try a launch pad →</Link></div>}
  </section>;
}
