...
export function TheoPractice() {
  ...
  return <section className="theo-practice lives-run" data-phase={phase} aria-labelledby="theo-title">
    <nav className="theo-toolbar" aria-label="Game navigation"><Link href="/lives/learn" aria-label="Back to learning"><ArrowLeft size={22} /></Link><span>Theo · Out the door</span>{phase === "playing" || phase === "paused" ? <button aria-label={phase === "paused" ? "Resume game" : "Pause game"} onClick={() => change(phase === "paused" ? "playing" : "paused")}>{phase === "paused" ? <Play size={22} /> : <Pause size={22} />}</button> : <span className="theo-toolbar-space" />}</nav>
    <header className="theo-heading"><span className="theo-eyebrow">A little less sidetracked</span><h1 id="theo-title" ref={heading} tabIndex={-1}>{phase === "ready" ? "Just get out the door." : phase === "success" ? "And you’re off!" : phase === "failure" ? "Still in the hallway." : phase === "paused" ? "The world can wait." : "Essentials. Then exit."}</h1></header>
    {phase === "ready" ? <div className="theo-intro"><div className="theo-intro-art"><TheoHallway /></div><div className="theo-intro-copy"><span className="theo-eyebrow">One mission. So many side quests.</span><h2>The plant can wait.</h2><p>Theo is ready to leave. Almost. Pack the essentials onto his launch pad, skip the tempting detours, then open the door.</p><p className="theo-rule">Three detours end the round. {still ? "Take your time." : `You have ${plan.duration / 1000} seconds.`}</p><label className="theo-setting">Challenge<select aria-label="Challenge" value={level} onChange={e => setLevel(Number(e.target.value))}><option value={1}>A usual morning · 3 essentials</option><option value={4}>A busier morning · 4 essentials</option><option value={8}>Everything at once · 5 essentials</option></select></label><label className="theo-untimed"><input type="checkbox" checked={still} disabled={reduced} onChange={e => setUntimed(e.target.checked)} /> No timer</label><button className="theo-primary" onClick={start}><Play size={20} weight="fill" /> Play Theo’s morning</button></div></div> : <>
      <div className="theo-time"><span>{still ? "Your pace. One thing at a time." : "Leave before time slips away"}</span><strong role="timer" aria-label="Time remaining">{still ? "No timer" : `${Math.max(0, Math.ceil((plan.duration - elapsed) / 1000))}s`}</strong></div>
