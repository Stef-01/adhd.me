// W105: ADHD.ME-staff access — the control the community interest register never had.
//
...
// and email. The register is a ADHD.ME-run community program, not practice data, so the correct
// control is "ADHD.ME staff only" — and the product had no way to say that.
//
// It is DELIBERATELY NOT A FOURTH ROLE. Adding `adhdme_staff` to `Role` would route it through
// `authorize()`, which resolves practice memberships — and then a practice owner using
// `manage_members` could grant ADHD.ME-staff access to themselves. That is the W97 lesson
...
// is ADHD.ME staff until the founder says who is, so today the answer is "nobody can read it",
...
/**
 * ADHD.ME staff. **Ships empty and a test pins it that way** — adding an entry is a founder
 * decision, made in a commit, not something a unit does in passing.
 */
export const ADHDME_STAFF: readonly StaffGrant[] = [];

function normalise(email: string): string {
  return email.trim().toLowerCase();
}

export function isAdhdMeStaff(
  email: string | null | undefined,
  grants: readonly StaffGrant[] = ADHDME_STAFF,
): boolean {
...
export const STAFF_REFUSAL_COPY =
  "This register belongs to the ADHD.ME community program, not to your practice, so it is not shown here. Nobody has been granted access to it yet.";
