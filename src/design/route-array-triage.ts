...
export const SPEC_TRIAGE: readonly SpecTriage[] = [
  ...
  { file: "app-shell.spec.ts", category: "single-route-feature", routes: ["/", "/story", "/examples", "/faq"], reason: "O230: the app's four tab routes, walked as one shell — the bar reaching each of them and marking the one it is on, the touch floor, and the sheet's dialog behaviour. It visits four routes because the shell IS four routes; it is not a sweep of the site." },
  { file: "consent.spec.ts", category: "single-route-feature", routes: ["/", "/story"], reason: "two named routes for one consent-banner feature (O230: the second is /story, the finder having taken the root)." },
