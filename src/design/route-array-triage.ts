...
export const SPEC_TRIAGE: readonly SpecTriage[] = [
  ...
  { file: "interface-coverage.spec.ts", category: "independently-derived", routes: [], reason: "O208. Holds no hardcoded array: it sweeps the surfaces that INTERFACE_STANCES marks `describes-both`, and that register is pinned against discoverSurfaces('app') in both directions by interface-coverage.test.ts — so the route list is derived, just through a register rather than through e2e/site-routes.ts. Same shape as public-sweep.spec.ts, which derives from PUBLIC_SURFACES for the same reason: the sweep is about a property of a CLASS of surface, not about every route." },
