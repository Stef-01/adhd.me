...
export const FOUNDER_DECISIONS: readonly FounderDecision[] = [
  {
    ...
    decision:
      "Yes — in the live finder, behind a visible opt-in tickbox, this deployment only. The founder's words (2026-09-01): “Yes and have a tikbox that will allow the synthetic profiles to follow. This finder system of the adhdme repo is not public so it’s ok it’s just for testing.” Read as: G-SYN-1 the finder itself, gated by a reader-visible checkbox rather than a route or env flag; G-SYN-2 finder repo only — the network tree does not take the personas; G-SYN-3 answered later the same day, overriding the plan's recommended eight: “Let’s put in 20 synthetic profiles”. Because the tickbox is an explicit opt-in to example data, synthetic entries rank NATURALLY among real ones while it is on — an artificial real-first order would defeat the testing purpose the founder named. AMENDED 2026-09-01 (same day, with the harmony review): “have the toggle by default have the synthetic drs and hide it away to be toggled at the voice entry section” — the examples ship ON, and the switch folds into the welcome screen's testing options as the way OFF. AMENDED AGAIN 2026-09-02 (O231, for the pitch demo): “it is demo so everything should be production ready looking, remember zero placeholders … If you add any weird placeholder sentences or note any profiles are synthetic you will ruin the entire pitch demo day we have been invited to.” Read as: the RENDERED LABEL goes — no `Example profile` badge on a results row, no fictional-GP sentence under a name, and no dead-end where the booking action belongs. The STRUCTURAL defences all stay and are not the founder's to waive by implication, because none of them is a label: exactly one of `realPerson`/`synthetic` on any rendered entry, `image: null` so no face is ever generated, no booking `url` so nothing opens a fabricated listing, no `disclosedInterest`, self-marking practice names, and the same patient-surface linter over every rendered string. One truthful sentence about the roster moved to `/story`, off the demo path entirely — the product may stop announcing that its sample data is sample data on every row, and may not assert that twenty invented GPs are a real network.",
