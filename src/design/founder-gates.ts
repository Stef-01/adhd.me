...
export const FOUNDER_DECISIONS: readonly FounderDecision[] = [
  {
    ...
    decision:
      "Yes — in the live finder, behind a visible opt-in tickbox, this deployment only. The founder's words (2026-09-01): “Yes and have a tikbox that will allow the synthetic profiles to follow. This finder system of the adhdme repo is not public so it’s ok it’s just for testing.” Read as: G-SYN-1 the finder itself, gated by a reader-visible checkbox rather than a route or env flag; G-SYN-2 finder repo only — the network tree does not take the personas; G-SYN-3 the plan's recommended eight. Because the tickbox is an explicit opt-in to example data, synthetic entries rank NATURALLY among real ones while it is on — an artificial real-first order would defeat the testing purpose the founder named.",
    decidedAt: "2026-09-01",
    effect:
      "src/demo/synthetic-roster.ts holds eight invented personas (synthetic: true, image null, booking via synthetic-none, no disclosed interests); the finder's results screen carries the tickbox, default OFF, and every surface that can show one labels it 'Example profile'. The plan's non-negotiables survive the decision: the `clinicians` export, ROSTER_SIZE and every count stay real-only, no booking route exists for a persona, no face is generated, and the copy passes the same patient linter as real entries — 'not public' loosened WHERE they render, not what invented copy may claim.",
  },
  {
