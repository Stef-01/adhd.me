...
export const EDUCATION_CONSOLE_COPY = {
  ...
  libraryAllShown:
    "ADHD.ME does not shorten this list. Every item about a register this practice runs is above, in full. A list that quietly dropped its weaker entries would be making a judgement on your behalf and hiding that it had made one.",

  /** W127's rule about zero states: say why it is empty rather than rendering a blank list. */
  libraryEmpty:
    "There is no material in the library. ADHD.ME ships with none of its own: every item has to trace back to content that cleared sign-off, and nothing has cleared it. That is the state of the product rather than a page that failed to load.",
  ...
  cpdEmpty: "No entries. ADHD.ME draws no conclusion from that.",

  cpdUnlinked:
    "Your sign-in is not linked to a clinician on this practice's roster, so ADHD.ME does not know whose reading record this would be. It will not guess.",
