...
const ORIGINAL_FLAG = process.env.ADHDME_ENABLE_MOCK_ROUTES;
...
beforeEach(() => {
  delete process.env.ADHDME_ENABLE_MOCK_ROUTES;
});

afterEach(() => {
  setNodeEnv(ORIGINAL_NODE_ENV);
  if (ORIGINAL_FLAG === undefined) delete process.env.ADHDME_ENABLE_MOCK_ROUTES;
  else process.env.ADHDME_ENABLE_MOCK_ROUTES = ORIGINAL_FLAG;
});
...
describe("mockRoutesEnabled", () => {
  ...
  it("is enabled in production only with the explicit opt-in flag", () => {
    setNodeEnv("production");
    process.env.ADHDME_ENABLE_MOCK_ROUTES = "1";
    expect(mockRoutesEnabled()).toBe(true);
  });
  ...
  it("requires exactly '1' for the opt-in", () => {
    ...
    for (const value of ["true", "yes", "0", ""]) {
      process.env.ADHDME_ENABLE_MOCK_ROUTES = value;
      expect(mockRoutesEnabled()).toBe(false);
    }
  });
});
